...
void* SJFcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            // Select the process with the shortest time
...
void* SRTFcpu(void* param) {
    ...
    while (1) {
        ...
        // lock to check if any process in the queue has a shorter remaining time
        pthread_mutex_lock(&(svars->readyQLock));
        ...
        // has nothing to run or when Q time is reached.
        if ((p != NULL)) {
            // check if any process in queue has a shorter remaining time
...
void* PPcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            int highestPriotityIndex = qPriority(&(svars->readyQ));
            p = qRemove(&(svars->readyQ), highestPriotityIndex);
